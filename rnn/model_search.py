...
class DARTSCellSearch(DARTSCell):
    def __init__(self, ninp, nhid, dropouth, dropoutx):
        super(DARTSCellSearch, self).__init__(
            ninp, nhid, dropouth, dropoutx, genotype=None
        )
    ...
    def cell(self, x, h_prev, x_mask, h_mask):
        ...
        for i in range(STEPS):
            ...
            ch = (
                masked_states.view(-1, self.nhid)
                .mm(self._Ws[i])
                .view(i + 1, -1, 2 * self.nhid)
            )
...
class RNNModelSearch(RNNModel):
    def __init__(self, *args):
        super(RNNModelSearch, self).__init__(
            *args, cell_cls=DARTSCellSearch, genotype=None
        )
        self._args = args
    ...
    def _initialize_arch_parameters(self):
        ...
        self.weights = torch.tensor(weights_data, requires_grad=True, device=self._args.device)
    ...
    def genotype(self):
        ...
        gene = _parse(F.softmax(self.weights, dim=-1).cpu().numpy())
